...
student-node ~ ➜  
``` kubectl exec -it -n ckad-pod-design custom-nginx -- cat /usr/share/nginx/html/index.html
Welcome to CKAD mock exams!
```
...
Once the services are created, you can edit the services to use the correct nodePorts as per the question using 
``` kubectl -n ns-ckad17-svcn edit svc lb2-ckad17-svcn. ```
...
``` kubectl auth can-i create pod --as mock-user ```

no

student-node ~ ✖ 

``` kubectl auth can-i create pod --as mock-user --namespace ckad20-auth-aecs ```
yes
...
student
-node ~ ➜  ``` kubectl apply -f service-3421-svcn.yaml  ```
